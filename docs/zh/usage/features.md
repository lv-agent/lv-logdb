...
`hash-chain`（`Config.hash_enabled`）在日志上附加一条防篡改的哈希链，使得对已封存 segment 的事后篡改都能在读取时被发现。它使用 **keyed 模式**的 BLAKE3：哈希链以一个每库唯一的 `hash_init`（32 字节 BLAKE3 密钥）作为种子，每条记录的哈希把前一条的哈希与记录正文链接起来，因此链上任意一字节被改动都会破坏其后所有记录的校验。

**`hash_init` 在数据库首次创建时由熵生成一次并持久化。** 新建数据库时，`generate_hash_init`（`src/lib.rs:685-699`）在 `open` 阶段用熵生成 32 字节密钥（`src/lib.rs:99-124`）。该密钥随后作为 `SegmentHeader` 的 `hash_init` 字段写入每个 segment 头部（`src/storage/format.rs:92-104`）——在创建每个新 segment 及每次滚动时盖上（`src/storage/mod.rs:526` 和 `:585`）。重启时，恢复逻辑从第一个有效 segment 头部把 `hash_init` 读回（`src/recovery.rs:165`），而**不会**重新生成。由此带来的影响：

- 读取时**可在崩溃或重启后重新校验**——读取方从 segment 头部恢复 `hash_init`，重新计算哈希链即可发现篡改。这正是密钥必须持久化的原因：因为它由熵生成，重启后若无法从盘上恢复密钥，链条就无法被重新校验。
- 防篡改能力来自**链结构本身，而非密钥的保密性。** `hash_init` 以明文形式存在于每个 segment 头部；读取文件的攻击者同样能看到它。链条所检出的是没有同时用该密钥重新计算链式哈希的损坏或原地篡改——已封存 segment 上任意一字节被改动都会破坏其后所有记录的校验，因为要重建一条自洽的链，必须从改动点开始向前重写每一个哈希。（若攻击者既能重写字节、又能用 `hash_init` 跑 keyed BLAKE3 重建出一致的链，那就能击败该链条；这超出了"防篡改封存"的能力范围，因此本特性并不构成像加密那样的安全边界。）
...
**公共 API——`LogDb::replicate(sequence, timestamp_ns, content)`。** 这是 `LogDb` 上**唯一**与远程相关的方法。它是 `logdbd` 备节点用于按主节点原始序写入的备节点写入路径，保留全局偏移空间，使消费方可以在主→备之间故障切换而无需重映射偏移。备节点契约（`src/lib.rs:326-391`）：
