...
pub async fn run(config: Config, mut sink: Box<dyn Sink>) -> Result<(), String> {
    ...
    let wm = source
        .get_watermark(&config.scope.namespace, &config.scope.stream)
        .await?;
    tracing::info!(
    ...
    let mut progress = Progress::load(&config.progress.file)?.unwrap_or_else(|| {
        Progress::new(
            wm.node_id.clone(),
            0,
            config.scope.namespace.clone(),
            config.scope.stream.clone(),
    ...
    if from <= wm.durable_seq {
        ...
        let chunks = source
            .scan(
                ...
                &config.scope.stream,
                from,
                config.pipeline.scan_batch_size as u32,
            )
            .await?;

        for chunk in &chunks {
            if !chunk.records.is_empty() {
                sink.push(&chunk.records)
                    .map_err(|e| format!("sink push: {}", e))?;
    ...
    loop {
        match source
            .tail(
                ...
                &config.scope.stream,
                progress.last_seq + 1,
                config.pipeline.tail_batch_size,
            )
            .await
        {
            Ok(mut stream) => {
                ...
                {
                    if resp.heartbeat || resp.records.is_empty() {
                        continue;
                    }
                    sink.push(&resp.records)
                        .map_err(|e| format!("sink push: {}", e))?;
