...
impl Source {
    pub async fn connect(addrs: &[String], tls_config: config::TlsConfig) -> Result<Self, String> {
        for (i, addr) in addrs.iter().enumerate() {
            match connect_addr(addr, &tls_config).await {
                Ok(client) => return Ok(Self { client, addrs: addrs.to_vec(), current_idx: i, tls_config }),
                Err(e) => {
    ...
    pub async fn failover(&mut self) -> Result<(), String> {
        ...
        self.client = connect_addr(addr, &self.tls_config).await
            .map_err(|e| format!("failover to {}: {}", addr, e))?;
        Ok(())
    }

    pub async fn get_watermark(&mut self, ns: &str, stream: &str) -> Result<Watermark, String> {
        let resp = self.client.get_watermark(GetWatermarkRequest {
            namespace: ns.into(), stream: stream.into(),
        }).await.map_err(|e| format!("get_watermark: {}", e))?;
        Ok(resp.into_inner())
    }

    pub async fn scan(&mut self, ns: &str, stream: &str, from: u64, limit: u32) -> Result<Vec<ScanResponse>, String> {
        let resp = self.client.scan(ScanRequest {
            namespace: ns.into(), stream: stream.into(),
            from_seq: from, to_seq: 0, limit,
        }).await.map_err(|e| format!("scan: {}", e))?;
        let mut stream = resp.into_inner();
        let mut results = Vec::new();
        while let Some(chunk) = stream.message().await.map_err(|e| format!("scan stream: {}", e))? {
            results.push(chunk);
        }
        Ok(results)
    }

    pub async fn tail(&mut self, ns: &str, stream: &str, from: u64, batch_size: u32) -> Result<tonic::Streaming<TailResponse>, String> {
        let resp = self.client.tail(TailRequest {
            namespace: ns.into(), stream: stream.into(),
            from_seq: from, batch_size,
            ..Default::default()
        }).await.map_err(|e| format!("tail: {}", e))?;
        Ok(resp.into_inner())
    }
}

async fn connect_addr(addr: &str, tls_config: &config::TlsConfig) -> Result<LogDbServiceClient<Channel>, String> {
    match tls_config.mode {
        config::TlsMode::Disabled => {
            let uri: tonic::transport::Uri = format!("http://{}", addr).parse()
        ...
        config::TlsMode::Tls => {
            ...
            let uri: tonic::transport::Uri = format!("https://{}", addr).parse()
                .map_err(|e| format!("invalid URI {}: {}", addr, e))?;
            let channel = Endpoint::from(uri).tls_config(tls)
