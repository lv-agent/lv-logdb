...
async fn start_logdbd() -> (String, tempfile::TempDir) {
    ...
    let svc = LogDbServiceImpl::new(storage, catalog, Arc::new(ConsumerTracker::new()), "test-node".into(), "primary".into());
...
async fn exporter_scans_existing_records() {
    ...
    // Write 5 records
    for i in 0..5u64 {
        client.append(append_req(format!("rec-{}", i).as_bytes())).await.unwrap();
    }
    // Wait for Committer
    for _ in 0..100 {
        let wm = client.get_watermark(logdbd::pb::GetWatermarkRequest {
            namespace: "test".into(), stream: "main".into(),
        }).await.unwrap().into_inner();
        if wm.durable_seq >= 5 { break; }
        tokio::time::sleep(Duration::from_millis(20)).await;
    }

    // Use exporter's source to scan
    let tls_config = logdb_exporter::config::TlsConfig::default();
    let mut source = logdb_exporter::source::Source::connect(&[addr.clone()], tls_config).await.unwrap();
...
async fn exporter_progress_roundtrip() {
    ...
    let mut prog = logdb_exporter::progress::Progress::new(
        "test-cluster".into(), 1, "test".into(), "main".into(), p.clone(),
    );
    prog.last_seq = 42;
    prog.save().unwrap();

    let loaded = logdb_exporter::progress::Progress::load(&p).unwrap().unwrap();
